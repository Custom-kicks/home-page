...
const MeetOurDesigners = () => {
  return (
    <section id="meet-our-designers" className="px-4 md:px-8 lg:px-16 py-16 bg-gray-50 border-b">
      ...
      <div className="flex flex-col lg:flex-row lg:justify-center lg:gap-16 gap-8">
        {designers.map((designer, index) => (
          <div 
            key={index} 
            className="flex flex-col items-center gap-4 bg-black rounded-md overflow-hidden"
            style={{
              width: '100%',
              maxWidth: '386px',
            }}
          >
            ...
            <div className="bg-black text-white w-full p-4">
              <h3 className="text-lg md:text-xl font-semibold">{designer.name}</h3>
              <p className="text-xs md:text-sm mt-2">{designer.testimony}</p>
            </div>

            {/* SVG Icon positioned below the combined content */}
